...
$base_url = elgg_normalize_url("$identifier/requests/$entity->guid") . '?' . parse_url(current_page_url(), PHP_URL_QUERY);

$list_class = (array) elgg_extract('list_class', $vars, array());
$list_class[] = 'elgg-list-membership-requests';

$item_class = (array) elgg_extract('item_class', $vars, array());
$item_class[] = 'elgg-membership-request';
...
$list_options = array(
	...
	'no_results' => elgg_echo("$identifier:membership_requests:no_results"),
	'pagination' => elgg_is_active_plugin('hypeLists') || !elgg_in_context('widgets'),
	'pagination_type' => 'default',
	'base_url' => $base_url,
	'list_id' => "membership-requests-$guid",
...
if (elgg_view_exists('lists/users')) {
	echo elgg_view('lists/users', array(
		...
		'rel' => 'membership_request',
		'group' => $entity,
	));
} else {
	$options['relationship'] = 'membership_request';
